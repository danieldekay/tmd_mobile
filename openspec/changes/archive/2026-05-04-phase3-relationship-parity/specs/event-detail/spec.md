...
The app SHALL provide a detail page at `/events/[id]` consuming TMD Core v3 `/tmd/v3/events/{id}`. All associated DJs, teachers, and series SHALL be rendered as navigation links where the corresponding detail routes exist.
...
#### Scenario: Event detail links to associated DJs
- **GIVEN** the event has one or more associated DJs
- **WHEN** the event detail page renders
- **THEN** each DJ name is a tappable link navigating to `/djs/[id]`

#### Scenario: Event detail links to event series
- **GIVEN** the event belongs to a series
- **WHEN** the event detail page renders
- **THEN** the series name is a tappable link navigating to `/event-series/[id]`
